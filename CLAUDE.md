...
- Local LLM tool calling (`llm_local.rs`, llama-cpp-2 0.1.150): the GGUF's embedded jinja chat template is rendered with **minijinja** (0.1.150 removed llama-cpp-2's OAI-compat/jinja chat layer). Tools are injected into the system prompt with a JSON output protocol; the reply is parsed leniently for tool calls in multiple formats (JSON object/array, OpenAI `tool_calls`, and Python/Llama `[name(arg=val)]`), after stripping `<think>` blocks. Fallbacks: system-role fold (gemma) → manual ChatML. Verified with gemma-4-E4B and LFM2.5-8B-A1B.
