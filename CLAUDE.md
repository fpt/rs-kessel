...
- Built-in tools (`create_default_registry`): `read`, `glob`, `grep`, `write`, `edit`, `multi_edit`, `bash`, `tasks`, `lookup_skill`, `read_situation_messages` (+ `capture_screen`/`find_window`/`apply_ocr` from `lib.rs`, + MCP tools)
