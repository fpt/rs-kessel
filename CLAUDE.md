...
| `lib/src/vm/` | Tiny fantasy-console stack VM (isa/vm/device/assembler/png) + a typed LL(1) Pascal/C-ish front-end (`uxlang.rs`, `.ux` → assembler) + `vm_*` tools for the model's write→assemble→run→observe→debug loop — registered **only** in `agent_new` (standalone app), absent from `kessel-cli`/app-server. See **[docs/VM.md](docs/VM.md)** |
