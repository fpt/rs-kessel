...
use crate::{samples_per_frame, Synth, SynthConfig, SynthStats};
    ...
    pub fn render(&mut self, out: &mut [f32]) {
        ...
        while done < total {
            while let Some(ev) = self.queue.take_due(self.now) {
                self.synth.handle(ev);
            }
            // Render up to the next event, or to the end of the block.
            let until = self.queue.next_time().unwrap_or(u64::MAX);
            let room = total - done;
