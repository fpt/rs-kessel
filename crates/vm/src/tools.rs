...
- Sprites are DECLARATIONS, not table literals: `sprite hero { <rows of chars, '.'=transparent else palette nibble 0-9a-f> }`. The SIZE COMES FROM THE BODY — rows are the height, characters the width — so 8 rows of 8 chars is one tile (draw it with `spr(hero, x, y, flags)`) and 16 rows of 16 chars is a 16x16 sprite you draw with `sprn(hero, x, y, flags)`. Do NOT hand-slice a big sprite into four 8x8 declarations; write it as one block that looks like the picture. Bigger than one tile, every row must be the same length and both sides a multiple of 8.
- Builtins: `cls(c)` (colour REQUIRED), `pset(x,y,c)`, `spr(id,x,y,flags)` (ONE 8x8 tile), `sprn(name,x,y,flags)` (a multi-tile sprite at its declared size — the usual way to draw one) or `sprn(id,x,y,w,h,flags)` (raw: a w×h block of contiguous tiles, for a run of ids you computed), `btn(LEFT|RIGHT|UP|DOWN|A|B)` (held), `btnp`/`btnr` (pressed/released THIS frame — use for jumps/menus), `frame_count()` (frames since start), `len(arr)` (array length), `clear(rec_or_arr)` (zero a record/array in place, e.g. reset a bullet pool), `text("LITERAL",x,y,color)` / `number(n,x,y,color)` (on-screen font: scores/titles/GAME OVER; `text` needs a string LITERAL), `sfx(id)` / `music(id)` / `music_stop()` (sound triggers), `entity(x,y,tag)` (report for observation), `rnd(n)`, `map/mget/mset/fset/solid` (tilemap).
